...
_PLE_PACKED_LAYOUT_CACHE: str | None = None

# E2M1 codebook (NVFP4 nibble -> float). CPU tensor at module scope; the
# layer registers it as a buffer at __init__ so CUDA graph capture only ever
# READS a device tensor — creating it inside the unpack would issue a
# CPU->CUDA copy at capture time, which torch refuses (unpinned) and which
# killed the first nvfp4 boot (09-20).
_E2M1_LUT_CPU = torch.tensor(
...
def _ple_packed_layout() -> str | None:
    """Sidecar table layout as seen by the GPU process (packed transport
    disambiguator). Reads META.json from VLLM_PLE_QUANT_DIR once — the dir is
    mounted ro into every container process, so worker (CPU) and this GPU
    unpack always agree on the same file."""
    global _PLE_PACKED_LAYOUT_CACHE
    if _PLE_PACKED_LAYOUT_CACHE is None:
        layout = ""
        try:
            import json
            with open(os.path.join(
                    os.environ.get("VLLM_PLE_QUANT_DIR", ""), "META.json"
            )) as fh:
                layout = str(json.load(fh).get("layout", ""))
        except Exception:
            layout = "?"
        _PLE_PACKED_LAYOUT_CACHE = layout
    return _PLE_PACKED_LAYOUT_CACHE
...
class Qwen3_8FlashNextNGramEmbedding(PleOffloadLayer):
    def __init__(
        ...
        _ple_sidecar = bool(os.environ.get("VLLM_PLE_QUANT_DIR"))
        if _ple_sidecar:
            with torch.device("meta"):
                self.ngram_embedding = VocabParallelEmbedding(
                    padded_vocab_size, self.head_dim,
                    params_dtype=params_dtype, padding_size=divisor,
    ...
    def get_offload_output_dtype(self, default_dtype: torch.dtype) -> torch.dtype:
        ...
        if os.environ.get("VLLM_PLE_PACKED", "0") == "1":
            # uint8 transport: [packed nibbles | fp16 group scales | pad] rows.
            return torch.uint8
...
class Qwen3_8FlashNextPLELayer(nn.Module, MambaBase):
    def __init__(
        ...
        self._ple_head_dim = int(config.ple_embed_dim) // (
            (self.short_conv_dilation - 1) * int(config.heads_per_ngram)
        )
        # nvfp4 arm (09-20): the codebook is computed inline from the row data
        # (see _unpack_packed_nvfp4) — NO constant tensor is registered, because
        # vLLM inits on the meta device + to_empty(): persistent=False buffers
        # not restored by load_weights would be UNINITIALIZED garbage there.
    ...
    def _unpack_packed_int4(
        self, embeddings: torch.Tensor, output_dtype: torch.dtype
    ) -> torch.Tensor:
        """GPU-side unpack of packed int4 PLE rows (group16, fp16 scales,
        low nibble first) — mirrors _PleQuantTable._dequant arithmetic
        exactly, so sidecar rows decode bit-identically to the CPU path."""
        hd = self._ple_head_dim
        rows = embeddings.reshape(-1, hd)
        pb = hd // 2
        sb = 2 * (hd // 16)
        packed = rows[:, :pb]
        lo = (packed & 0xF).to(torch.float32)
        hi = (packed >> 4).to(torch.float32)
        nib = torch.stack((lo, hi), dim=-1).reshape(rows.shape[0], hd)
        sc = rows[:, pb:pb + sb].contiguous().view(torch.float16).to(torch.float32)
        x = (nib - 8.0) * sc.repeat_interleave(16, dim=1)
        return x.view(embeddings.shape).to(output_dtype)

    def _unpack_packed_nvfp4(
        self, embeddings: torch.Tensor, output_dtype: torch.dtype
    ) -> torch.Tensor:
        """GPU-side unpack of packed NVFP4 PLE rows (e2m1 codes, e4m3
        group-16 scales, per-row fp32 global scale — see worker.py wire
        note). Arithmetic mirrors the PR #56273 Triton kernel
        `lut[nib] * scales * scale2` and the CPU _PleQuantTable e2m1
        branch (same LUT, same nibble order, same group mapping)."""
        hd = self._ple_head_dim
        rows = embeddings.reshape(-1, hd)
        pb = hd // 2
        nsc = hd // 16
        packed = rows[:, :pb]
        # E2M1 decode, arithmetic-only (bit patterns exactly mirror upstream
        # nvfp4_emulation_utils._e2m1_inline): magnitude bits 0..7 of the nibble
        # via IEEE754 construction, sign in bit 3. Python scalars only — safe
        # under CUDA-graph capture (no H2D copy, no registered constant).
        lo = packed & 0xF
        hi = (packed >> 4) & 0xF
        nib = torch.stack((lo, hi), dim=-1).reshape(rows.shape[0], hd).to(torch.int32)
        mag = nib & 0x7
        sign = (nib >> 3) & 1
        val = (1056964608 + (mag << 22)).view(torch.float32)  # 0x3F000000 base
        val = torch.where(mag == 0, 0.0, val)
        val = torch.where(mag == 1, 0.5, val)
        nib_f = torch.where(sign == 1, -val, val)
        sc = (rows[:, pb:pb + nsc].clone()
              .view(torch.float8_e4m3fn).to(torch.float32))
        g = rows[:, pb + nsc:pb + nsc + 4].clone().view(torch.float32)
        x = nib_f * sc.repeat_interleave(16, dim=1) * g[:, :1]
        return x.view(embeddings.shape).to(output_dtype)
    ...
    def _dequantize_embeddings(
        ...
        if embeddings.dtype == torch.uint8:
            if "e2m1" in (_ple_packed_layout() or ""):
                return self._unpack_packed_nvfp4(embeddings, output_dtype)
            return self._unpack_packed_int4(embeddings, output_dtype)
