...
class CustomAllreduce:
    ...
    def __init__(
        ...
        # Rig patch: VLLM_CUSTOM_AR_ALLOW_PCIE=1 keeps custom allreduce on for
        # >2 PCIe-only GPUs (no NVLink). Upstream disables it there because
        # its one-shot/two-shot kernels assume NVLink bandwidth; this rig has
        # driver-level P2P at ~25 GB/s per pair, so it is a measurement, not
        # a given. The P2P self-test below still has to pass.
        if (
            same_node
            and world_size > 2
            and not fully_connected
            and os.environ.get("VLLM_CUSTOM_AR_ALLOW_PCIE", "0") == "1"
        ):
            logger.warning(
                "Custom allreduce kept ENABLED on %d PCIe-only GPUs by "
                "VLLM_CUSTOM_AR_ALLOW_PCIE=1 (rig patch).",
                world_size,
            )
