...
_USE_CUSTOM_ALL_GATHER = _os.getenv("VLLM_CUSTOM_ALL_GATHER", "0") == "1"
...
class CudaCommunicator(DeviceCommunicatorBase):
    ...
    def all_gather(self, input_: torch.Tensor, dim: int = -1) -> torch.Tensor:
        ...
        # QWEN-PEDIA r10 port (saturn): route small all-gathers through the
        # custom one-shot IPC all-gather kernel that ships next to custom
        # all-reduce (never reached from here upstream). On 4x PCIe 5090s the
        # target's 2 MB logits gather cost ~0.5 ms/step on NCCL; the P2P
        # kernel reads peers' buffers directly. Opt in with
        # VLLM_CUSTOM_ALL_GATHER=1; anything the kernel refuses (size > its
        # buffer, dtype) falls through to NCCL unchanged.
        ca_comm = self.ca_comm
        if (
            _USE_CUSTOM_ALL_GATHER
            and ca_comm is not None
            and not ca_comm.disabled
        ):
            input_size = input_.size()
            out = ca_comm.custom_all_gather(input_.contiguous())
            if out is not None:
