...
class ModelCudaGraphManager(CudaGraphManager):
    ...
    def capture(
        ...
        progress_bar_desc: str = "Capturing CUDA graphs",
        pre_forward_hook: Callable[[int], None] | None = None,
    ) -> None:
        ...
        def create_forward_fn(
            ...
            def forward_fn(cg_mode: CUDAGraphMode) -> None:
                # Each eager warmup consumes the PLE ready flag. Re-arm it for
                # every shape; signalling once before the loop leaves the
                # second warmup waiting for a CPU request that was never sent.
                if pre_forward_hook is not None:
                    pre_forward_hook(num_tokens)
