...
# ============================================================================
# === mixvmm arm (09-23, WORKTREE fn-ext only) ===============================
# The live AutoGPTQ shim (maybe_init_static_hot_cache @932) delegates to the
# WNA16 method with self=AutoGPTQMoEMethod; when DYNAMIC_LRU=0 the WNA16 gate
# calls self.maybe_init_mixed_vmm_hot_cache which exists ONLY on the WNA16
# class -> AttributeError (this exact crash, first boot). Borrow it, and after
# VMM permutes/replaces the generic-named parameters, re-alias the AutoRound
# names the Humming kernel actually reads (w13_qweight/w13_scales @746-749)
# onto the new tensors, then rebuild the kernel config. Without the re-alias
# the arm would silently compute on stale (unpermuted, un-tiered) tensors.
# ============================================================================
from vllm.model_executor.layers.quantization.compressed_tensors.compressed_tensors_moe import compressed_tensors_moe_wna16 as _wna16_mod
_WNA16_CLS = _wna16_mod.CompressedTensorsWNA16MoEMethod
_orig_vmm_init = _WNA16_CLS.__dict__["maybe_init_mixed_vmm_hot_cache"]

def _mixvmm_init_on_autogptq(self, layer):
    _orig_vmm_init(self, layer)
    if not getattr(self, "_mixed_vmm_active", False):
        return
    # post-VMM re-alias: generic (replaced) names -> AutoRound kernel names
    _pairs = (
        ("w13_weight_packed", "w13_qweight"), ("w2_weight_packed", "w2_qweight"),
        ("w13_weight_scale", "w13_scales"), ("w2_weight_scale", "w2_scales"),
        ("w13_weight_zero_point", "w13_qzeros"), ("w2_weight_zero_point", "w2_qzeros"),
    )
    for _new, _auto in _pairs:
        _v = getattr(layer, _new, None)
        if _v is not None and getattr(layer, _auto, None) is not _v:
            setattr(layer, _auto, _v)
    layer.w13_weight = layer.w13_weight_packed
    layer.w2_weight = layer.w2_weight_packed
    self._setup_kernel(layer)

AutoGPTQMoEMethod.maybe_init_mixed_vmm_hot_cache = _mixvmm_init_on_autogptq
for _n in ("_allocate_mixed_vmm_tensor", "_cuda_driver_check"):
    _a = _WNA16_CLS.__dict__.get(_n)
    if _a is not None:
        setattr(AutoGPTQMoEMethod, _n, _a)
print("[mixvmm] AutoGPTQ<-WNA16 VMM shim bound (re-aliasing on)", flush=True)
